...
const regularAppIconName = 'icon_default.png';
const regularAppSplashName = 'splash_default.png';
...
console.log('Setting up icon and splash screen resources for DEFAULT app...');
copyFileSync(path.join('resources', regularAppIconName), 'resources/icon.png');
copyFileSync(
  path.join('resources', regularAppSplashName),
...
function copyImages(sourcePath, targetPath, images) {
  try {
    for (const image of images) {
      const source = path.join(sourcePath, image.source);
      const target = path.join(targetPath, image.target);
...
const generateIOS = () => {
  console.log('Generating resources for iOS...');
...
const generateAndroid = () => {
  console.log('Generating resources for Android...');
...
if (args.includes('--android')) {
  ...
  console.log('No platform specified. Use --android, --ios, or --all.');
}
